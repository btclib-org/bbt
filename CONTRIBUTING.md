...
`package = false`, this tree being course material rather than a
package — notebooks, spreadsheets, a regtest walk-through and a
...
which is a diff to read rather than to commit unattended.
...
The two disabled plugins are the hook's own comment to explain; passing
them here is what keeps a regenerated baseline the same baseline.
